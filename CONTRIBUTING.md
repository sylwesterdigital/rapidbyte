...
We welcome contributions of all kinds — bug fixes, new connectors, documentation improvements, and test coverage. Whether you are fixing a typo or building a full connector, thank you for helping make Rapidbyte better.
...
Connectors are independent Wasm components that compile to `wasm32-wasip2` and depend only on `rapidbyte-sdk`. They do not require any engine changes. See [docs/CONNECTOR_DEV.md](docs/CONNECTOR_DEV.md) for a complete walkthrough covering the SDK traits, build setup, configuration, and testing.
...
- **Error handling** — Use `ConnectorError` factories at crate boundaries; use `thiserror` for internal error types.
...
| `just lint` | Always |
| `just test` | Always |
| `just e2e` | Engine, runtime, or connector changes |
...
connectors/
  source-postgres/    # wasm32-wasip2
  dest-postgres/      # wasm32-wasip2
  transform-sql/      # wasm32-wasip2
  transform-validate/ # wasm32-wasip2
```

**Dependency chain:** types &larr; state &larr; runtime &larr; engine &larr; cli. Connectors depend only on `rapidbyte-sdk`.
