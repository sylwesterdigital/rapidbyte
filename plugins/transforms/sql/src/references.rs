...
fn query_external_table_references(query: &str) -> Result<BTreeSet<String>, String> {
    ...
    let statements = datafusion::sql::sqlparser::parser::Parser::parse_sql(&dialect, query)
...
fn collect_set_expr_external_table_references(
    ...
    match set_expr {
        SetExpr::Select(select) => {
            collect_select_external_table_references(select, scope, references);
        }
...
#[cfg(test)]
mod tests {
    use super::validate_query_for_stream_name;

    #[test]
    fn validate_succeeds_for_qualified_current_stream_name() {
        let result = validate_query_for_stream_name("SELECT id FROM public.users", "public.users");
        assert!(result.is_ok());
    }

    #[test]
    fn validate_rejects_queries_that_reference_multiple_external_tables() {
        let err = validate_query_for_stream_name(
            "SELECT users.id FROM users JOIN orders ON users.id = orders.user_id",
            "users",
        )
        .expect_err("validation should fail");

        assert!(err.contains("may only reference current stream table"));
        assert!(err.contains("orders"));
    }

    #[test]
    fn validate_allows_ctes_derived_from_current_stream() {
        let result = validate_query_for_stream_name(
            "WITH filtered AS (SELECT id FROM users) SELECT id FROM filtered",
            "users",
        );
    ...
    #[test]
    fn validate_rejects_cte_aliases_that_shadow_disallowed_tables() {
        let err = validate_query_for_stream_name(
            "WITH orders AS (SELECT * FROM orders) SELECT * FROM users",
            "users",
        )
        .expect_err("validation should fail");
    ...
    #[test]
    fn validate_allows_quoted_current_stream_identifier() {
        let result = validate_query_for_stream_name(r#"SELECT id FROM "users""#, "users");
        assert!(result.is_ok());
    }

    #[test]
    fn validate_rejects_cross_stream_subquery_references() {
        let err = validate_query_for_stream_name(
    ...
    #[test]
    fn validate_fails_for_legacy_input_table_name() {
        let err = validate_query_for_stream_name("SELECT id FROM input", "users")
            .expect_err("validation should fail");
